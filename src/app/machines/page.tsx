...
function MachinesContent() {
    ...
    try {
      ...
      if (saved) {
        ...
        if (loc.city) {
          ...
          setSiteDisplay(loc.display || `${loc.city}${loc.state ? `, ${loc.state}` : ""}`);
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        try {
          ...
          if (detectedCity) {
            const display = `${detectedCity}${detectedState ? `, ${detectedState}` : ""}`;
            setSiteCity(detectedCity);
            setSiteState(detectedState);
            setSiteDisplay(display);
            localStorage.setItem(SITE_LOCATION_KEY, JSON.stringify({ city: detectedCity, state: detectedState, display }));
          } else {
            setShowLocationBanner(true);
          }
        } catch {
  ...
  const fetchMachines = useCallback(async (userCity?: string, userState?: string) => {
    ...
    setLoading(false);
  }, [type, search, siteCity, siteState]);
  ...
  const handleLocationSelect = (loc: LocationResult) => {
    setSiteCity(loc.city);
    setSiteState(loc.state);
    setSiteDisplay(loc.displayName);
    setShowLocationBanner(false);
    localStorage.setItem(SITE_LOCATION_KEY, JSON.stringify({ city: loc.city, state: loc.state, display: loc.displayName }));
    fetchMachines(loc.city, loc.state);
  };
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
        ...
        <div className="bg-gradient-to-r from-slate-900 to-slate-800 rounded-xl p-4 sm:p-5 mb-5 text-white">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              <h3 className="font-bold text-sm sm:text-base mb-1">Where&apos;s your project site?</h3>
              <p className="text-xs sm:text-sm text-slate-400 mb-3">Search an address, area, or city to find nearby machines</p>
      ...
      {siteCity && !showLocationBanner && !detectingLocation && (
        <div className="flex items-center gap-2 mb-4 text-sm">
          <span className="bg-gradient-to-r from-amber-100 to-orange-100 text-amber-800 px-3 py-1.5 rounded-full font-medium text-xs sm:text-sm truncate max-w-[280px] sm:max-w-none flex items-center gap-1.5 shadow-sm">
            <svg className="w-3.5 h-3.5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
            {siteDisplay || siteCity}
          </span>
          <button
            onClick={() => { setShowLocationBanner(true); setLocationQuery(""); }}
      ...
      {loading ? (
        ...
        <div className="text-center py-16 sm:py-20 bg-white rounded-2xl border border-slate-200">
          ...
          <button onClick={() => { setType("all"); setSearch(""); setTimeout(() => fetchMachines(), 100); }} className="text-amber-600 font-semibold hover:underline text-sm">
            Clear All Filters
          </button>
        </div>
      ) : (
