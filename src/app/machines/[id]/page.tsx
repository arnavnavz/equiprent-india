...
export default function MachineDetailPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <Link href="/machines" className="inline-flex items-center text-sm text-slate-500 hover:text-slate-700 mb-6">
        ← Back to Machines
      </Link>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div>
            <div className="flex items-start justify-between gap-4 mb-4">
              <div>
                <span className="inline-flex items-center gap-1 bg-amber-50 text-amber-700 text-sm font-semibold px-3 py-1 rounded-full mb-2">
                  {getMachineTypeIcon(machine.type)} {getMachineTypeLabel(machine.type)}
                </span>
                <h1 className="text-2xl sm:text-3xl font-bold text-slate-900">{machine.name}</h1>
                <p className="text-slate-500 mt-1">{machine.brand} {machine.model} • {machine.year}</p>
              </div>
            ...
            <div className="flex flex-wrap gap-3 mb-6">
              {machine.operatorIncluded && (
                <span className="bg-green-50 text-green-700 text-sm font-medium px-3 py-1.5 rounded-lg">✅ Operator Included</span>
              )}
              {machine.fuelIncluded && (
                <span className="bg-blue-50 text-blue-700 text-sm font-medium px-3 py-1.5 rounded-lg">⛽ Fuel Included</span>
              )}
              <span className="bg-slate-100 text-slate-700 text-sm font-medium px-3 py-1.5 rounded-lg">
                📍 {machine.city}, {machine.state}
              </span>
              {machine.avgRating > 0 && (
                <span className="bg-amber-50 text-amber-700 text-sm font-medium px-3 py-1.5 rounded-lg">
                  ★ {machine.avgRating.toFixed(1)} ({machine.reviewCount} reviews)
                </span>
              )}
            </div>
            ...
            <div className="bg-white rounded-xl border border-slate-200 p-6 mb-6">
              ...
              <p className="text-slate-600 leading-relaxed whitespace-pre-line">{machine.description}</p>
            </div>

            {(machine.capacity || machine.specifications) && (
              <div className="bg-white rounded-xl border border-slate-200 p-6 mb-6">
                <h3 className="font-bold text-slate-900 mb-3">Specifications</h3>
                {machine.capacity && (
                  <p className="text-slate-600 mb-2"><span className="font-medium">Capacity:</span> {machine.capacity}</p>
                )}
                {machine.specifications && (
                  <p className="text-slate-600 whitespace-pre-line">{machine.specifications}</p>
                )}
              </div>
            )}
            ...
            {machine.reviews.length > 0 && (
              <div className="bg-white rounded-xl border border-slate-200 p-6">
                <h3 className="font-bold text-slate-900 mb-4">Reviews ({machine.reviewCount})</h3>
          ...
          <div className="sticky top-24">
            {bookingSuccess ? (
              <div className="bg-green-50 border border-green-200 rounded-2xl p-8 text-center">
                ...
                <h3 className="text-xl font-bold text-green-800 mb-2">Booking Requested!</h3>
                <p className="text-green-700 text-sm mb-6">
                  The owner will review and confirm your booking. You&apos;ll be notified once confirmed.
                </p>
              ...
              <form onSubmit={handleBooking} className="bg-white rounded-2xl border border-slate-200 p-6 space-y-4">
                <h3 className="text-lg font-bold text-slate-900">Book This Machine</h3>

                {bookingError && (
                  <div className="bg-red-50 text-red-700 px-4 py-3 rounded-lg text-sm">{bookingError}</div>
                )}
                ...
                {user?.id === machine.owner.id ? (
                  ...
                  <>
                    ...
                    <div>
                      <label className="block text-sm font-semibold text-slate-700 mb-1">Project Site Address</label>
                      <input
                        type="text"
                    ...
                    {!user ? (
                      <Link
                        href="/login"
                        className="block w-full bg-amber-500 text-white py-2.5 rounded-lg font-semibold hover:bg-amber-600 transition-colors text-sm text-center"
                      >
                        Sign In to Book
                      </Link>
                    ) : (
